 ...
 */
public class MyTreeSet2<T extends Comparable<? extends T>>{
    private static class BinaryNode<T> {
        ...
        public BinaryNode(T element, BinaryNode<T> lt, BinaryNode<T> rt, BinaryNode<T> next, BinaryNode<T> prev) {
            this.element = element;
            this.left = lt;
            this.right = rt;
            this.next = next;
    ...
    private BinaryNode<T> insert(T x, BinaryNode<T> t, BinaryNode<T> next, BinaryNode<T> prev) {
        if (t == null) {
            modCount++;
        ...
        if (compareResult < 0) {
           t.left = insert(x, t.left, t, prev);
            ...
            t.right = insert(x, t.right, next, t);
        } else {
            ;
        }
        return t;
